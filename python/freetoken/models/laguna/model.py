...
from .moe import LagunaSparseBlock
...
class LagunaDecoderLayer(BaseOP):
    def __init__(self, config: ModelConfig, layer_id: int):
        ...
        if layer_id >= config.first_k_dense_replace:
            self.mlp: BaseOP = LagunaSparseBlock(config, layer_id)
        else:
            # Leading dense layer: BF16 even on the NVFP4 checkpoint (quantization ``ignore``).
            self.mlp = LagunaGatedMLP(config.hidden_size, config.intermediate_size)
...
class LagunaModel(BaseOP):
    def __init__(self, config: ModelConfig):
        ...
        self.layers = OPList(
            [LagunaDecoderLayer(config, layer_id) for layer_id in range(config.num_layers)]
        )
